...
export async function recordVisit(id: string): Promise<void> {
  ...
  if (database) {
    try {
      // Cek dulu apakah page ada
      const pageSnap = await get(ref(database, "pages/" + id + "/title"));
      if (!pageSnap.exists()) return;

      // Update views & uniqueVisitors langsung ke field-nya (tidak overwrite seluruh page)
      const viewsRef = ref(database, "pages/" + id + "/views");
      const viewsSnap = await get(viewsRef);
      const currentViews = (viewsSnap.val() as number) || 0;
      await set(viewsRef, currentViews + 1);

      if (!alreadyVisited) {
        const uvRef = ref(database, "pages/" + id + "/uniqueVisitors");
        const uvSnap = await get(uvRef);
        const currentUV = (uvSnap.val() as number) || 0;
        await set(uvRef, currentUV + 1);
      }

      // Simpan visit log di path terpisah agar tidak mengganggu data page
      try {
        const visitKey = String(Date.now()) + "_" + visitorId.slice(0, 6);
        await set(ref(database, "visits/" + id + "/" + visitKey), {
          ts: Date.now(),
...
export async function getVisitLog(id: string): Promise<{ ts: number; ua?: string }[]> {
  ...
  if (database) {
    try {
      const snap = await get(ref(database, "visits/" + id));
      if (!snap.exists()) return [];
