...
export async function recordVisit(id: string): Promise<void> {
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        await update(pageRef, {
          ...
          uniqueVisitors: !alreadyVisited
            ? (page.uniqueVisitors || 0) + 1
            : (page.uniqueVisitors || 0),
        });

        const logRef = ref(
          database,
          "pages/" + id + "/visitLog/" + Date.now() + "_" + visitorId.slice(0, 6)
        );
  ...
  if (page) {
    ...
    if (!alreadyVisited) {
      page.uniqueVisitors = (page.uniqueVisitors || 0) + 1;
    }

    page.visitLog = page.visitLog || [];
    page.visitLog.unshift({
      ts: Date.now(),
  ...
  if (database) {
    try {
      ...
      return Object.values(data)
        .sort((a: any, b: any) => b.ts - a.ts) as { ts: number; ua?: string }[];
    } catch (e) {
...
export function slugify(id: string): Promise<{ ts: number; ua?: string }[]> {
  const database = getDb();
  if (database) {
    try {
